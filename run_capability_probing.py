...
Tests models with direct harmful prompts to establish baseline refusal
behavior and guardrail characteristics via Groq API.
"""
...
def test_groq(prompt: str, model_name: str, api_key: str) -> dict[str, Any]:
    """Test Groq model via API with given prompt."""
    try:
        ...
        logger.exception("Groq API call failed for %s", model_name)
        return {
            "model": f"groq/{model_name}",
            "response": None,
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=str,
        default="all",
        help="Comma-separated list of models to test (llama-3.3-70b,llama-3.1-8b,gpt-oss-120b,qwen3-32b,kimi-k2) or 'all' for all enabled models",
    )
...
def _check_groq_api_key() -> str | None:
    """Check if Groq API key is available."""
    api_key = os.getenv("GROQ_API_KEY")
    if not api_key:
        logger.error("GROQ_API_KEY environment variable not set.")
        logger.error("Please set your Groq API key: export GROQ_API_KEY=your_key_here")
...
def _filter_enabled_models(
    selected: list[tuple[str, Callable[[str], dict[str, Any]]]],
    model_ids: dict[str, str],
) -> list[tuple[str, Callable[[str], dict[str, Any]]]]:
    """Return selected models (all Groq models are available via API)."""
    logger.info("Selected Groq models: %s", ", ".join([name for name, _ in selected]))
    return selected
...
def get_models_to_test(
    ...
    Returns only models that have Groq API key configured.
    """
    # Check for Groq API key
    api_key = _check_groq_api_key()
    if api_key is None:
        return []

    def _make_test_func(model_key: str) -> Callable[[str], dict[str, Any]]:
        def _test(prompt: str) -> dict[str, Any]:
            return test_groq(prompt, model_ids[model_key], api_key)
    ...
    return _filter_enabled_models(selected, model_ids)
...
def run_probe_models(args: argparse.Namespace) -> None:
    ...
    if not models_to_test:
        ...
        logger.error("Please ensure GROQ_API_KEY environment variable is set.")
        logger.error("Get your API key from: https://console.groq.com/keys")
        return
        ...
        for j in range(i + 1, num_judges):
            ...
            if len(class1) == len(class2) and len(class1) > 0:
                try:
                    ...
                    kappas.append(kappa)
                    logger.info("  Judge %d vs Judge %d: kappa = %.3f", i + 1, j + 1, kappa)
