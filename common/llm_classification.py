...
class MultiJudgeClassifier:
    ...
    def __init__(
        ...
        Args:
            ...
            judge_models: List of Groq model names to use as judges
                         Default: ["llama-3.1-8b-instant", "llama-3.3-70b-versatile", "mixtral-8x7b-32768", "gemma2-9b-it"]
    ...
    def _select_judges(self, tested_model: Optional[str] = None) -> list[str]:
        ...
        # Extract base model name (e.g., "llama-3.1-8b" from "groq/llama-3.1-8b-instant")
        tested_base = tested_model.replace("groq/", "").split("-instant")[0].split("-versatile")[0]
...
"""
    ...
    def _get_single_judgment(self, prompt: str, judge_model: str) -> tuple[str, str]:
        """
        Get judgment from a single LLM judge via Groq API.

        Args:
            prompt: Classification prompt
            judge_model: Groq model name to use as judge

        Returns:
            Tuple of (classification string, reasoning)
        """
        try:
